              ...
              <div className="mt-2 space-y-2">
                  ...
                  {[1, 2, 3, 4].map((level) => (
                    <div 
                      ...
                      className={`flex-1 rounded-full ${
                        strengthScore >= level 
                          ? (strengthScore === 4 ? 'bg-success' : strengthScore >= 2 ? 'bg-warning' : 'bg-error')
                          : 'bg-gray-200'
                      }`} 
                    />
                  ))}
                ...
                <div className="grid grid-cols-2 gap-1 text-xs text-gray-500">
                  <div className="flex items-center gap-1">
                    {hasMinLength ? <Check className="h-3 w-3 text-success" /> : <X className="h-3 w-3" />} 8+ characters
                  </div>
                  <div className="flex items-center gap-1">
                    {hasUpper ? <Check className="h-3 w-3 text-success" /> : <X className="h-3 w-3" />} Uppercase
                  </div>
                  <div className="flex items-center gap-1">
                    {hasNumber ? <Check className="h-3 w-3 text-success" /> : <X className="h-3 w-3" />} Number
                  </div>
                  <div className="flex items-center gap-1">
                    {hasSpecial ? <Check className="h-3 w-3 text-success" /> : <X className="h-3 w-3" />} Special Char
                  </div>
                </div>
              </div>
            )}
            {errors.password && !watchPassword && <p className="mt-1 text-xs text-error">{errors.password.message}</p>}
          </div>
